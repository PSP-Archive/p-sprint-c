...
int p_spGetKeycodeFriendlyName(int keyCode, char keyName[20])
{
	...
	case 93:
		...
		break;
	case 164:
		strcpy(keyName,"L_MENU");
		break;
	case 165:
		strcpy(keyName,"R_MENU");
		break;
	case 162:
		strcpy(keyName,"L_CTRL");
		break;
	case 163:
		strcpy(keyName,"R_CTRL");
		break;
	case 160:
		strcpy(keyName,"L_SHFT");
		break;
	case 161:
		strcpy(keyName,"R_SHFT");
		break;
	case 13:
...
int p_spReadKey(struct p_sp_Key * newKey)
{
	/* main p_sprint function:
	...
	SceCtrlData pad;
	sceCtrlReadBufferPositive(&pad, 1);
	return p_spReadKeyEx(newKey,pad.Buttons);
}
...
int p_spGetKeyCode(void)
{
	...
	/* reads one key and returns its keycode
	This function is a blocking call */
	while(!keyCode)
	{
		p_spReadKey(&myKey);
		keyCode = myKey.keycode;
	}
	return keyCode;
}
...
char p_spGetChar(void)
{
	int keyChar = 0;
	struct p_sp_Key myKey;
	/* reads one key and returns its character value
	This function is a blocking call */
	while(!keyChar)
	{
		p_spReadKey(&myKey);
		keyChar = myKey.keychar;
	}
	return keyChar;
}

int p_spGetKey(struct p_sp_Key * newKey)
{
	int keyId = 0;
	struct p_sp_Key myKey;
	/* returns a key structure
	This function is a blocking call */
	while(!keyId)
	{
		p_spReadKey(&myKey);
